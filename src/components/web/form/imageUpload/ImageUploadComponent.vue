    ...
    <div class="upload-card-list">
      <div class="file-list" v-for="(item, index) in fileList" :key="index">
        <el-image
          class="img-item"
          fit="contain"
          :src="$setAssetType(item.url)"
          :preview-teleported="true"
          :preview-src-list="previewList"
        />
        <span class="image-wrap">
          <el-icon @click.stop="downloadImage(item)">
            <Download />
          </el-icon>
          <el-icon
...
<script setup>
import { computed, watch, ref, getCurrentInstance, inject, nextTick } from "vue";
...
import { IMAGE_UPLOAD_TYPE_ITEM } from "@/config/pageDesign/componentConfig";
...
const customClass = computed(() => {
  return `custom-image-upload-${props.compSize} image-upload-box`;
});
const imageTypeItem = computed(() => IMAGE_UPLOAD_TYPE_ITEM);
...
function beforeUpload(file) {
  ...
  if (props.limitTypeArray.includes("全部") || !props.limitTypeArray.length) {
    const limitFlag = imageTypeItem.value.some(item => {
  ...
  if (props.limitTypeArray.length) {
    const limitFlag = props.limitTypeArray.some(type => {
      return suffix === type;
    });
    if (!limitFlag) {
      proxy.$message.warning("不支持该类型的文件上传");
...
.custom-image-upload-superLarge {
  :deep(.el-upload--picture-card) {
    width: 200px;
    height: 200px;
    line-height: 200px;
