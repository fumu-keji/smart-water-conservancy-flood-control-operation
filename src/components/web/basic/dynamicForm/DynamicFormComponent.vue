<template>
  <div class="dynamic-form-box table-render-box">
    <el-form
...
const emit = defineEmits(["rowClick", "changeTableData"]);
/**
 * 配置相关
 */
...
watch(() => props.tableData, val => {
  if (!isEmpty(val)) {
    if (JSON.stringify(formTableData.tableList) === val) {
      return;
    }
    formTableData.tableList = JSON.parse(val);
  } else {
    formTableData.tableList = [];
  }
}, { immediate: true, deep: true });
watch(() => formTableData.tableList, val => {
  emit("changeTableData", JSON.stringify(val));
}, { deep: true });
watch(() => props.dynamicFormColumnData, val => {
  if (val) {
    initTableColumnData();
  }
}, { immediate: true, deep: true });
...
async function initTableColumnData() {
  tableColumnData.value = JSON.parse(JSON.stringify(props.dynamicFormColumnData));
  let componentName = {
