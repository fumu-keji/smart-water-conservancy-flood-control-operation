...
let defaultAttrs = computed(() => {
  return Object.assign({}, defaultEchartAttribute, lineAttribute);
});
...
async function initChart(resResultItem) {
  ...
  await setLineData(options, resResultItem, resultAttrs);
  renderOptions.value = options;
  renderModel.value = resultAttrs.darkModel ? "dark" : "light";
  renderRef.value.initRenderChart();
}
// 设置面积图的各个值
async function setLineData(options, resResultItem, resultAttrs) {
  let resultItem = [];
