...
const doughnutAttribute = {
  legendOrient: "vertical",
  doughnutInnerRadius: "40",
  doughnutOutSideRadius: "70",
  roundedModel: true,
  showLabel: true,
  labelPosition: "outer",
  labelFontSize: "16",
...
let defaultAttrs = computed(() => {
  return Object.assign({}, defaultEchartAttribute, doughnutAttribute);
});
...
async function initChart(resResultItem) {
  ...
  await setDoughnutData(options, resResultItem, resultAttrs);
  renderOptions.value = options;
  renderModel.value = resultAttrs.darkModel ? "dark" : "light";
  renderRef.value.initRenderChart();
}
// 设置面积图的各个值
async function setDoughnutData(options, resResultItem, resultAttrs) {
  let resultItem = [];
  ...
  // 设置统计和整体环图的数据
  let doughnutInnerRadius = resultAttrs.doughnutInnerRadius ? resultAttrs.doughnutInnerRadius : "40";
  let doughnutOutSideRadius = resultAttrs.doughnutOutSideRadius ? resultAttrs.doughnutOutSideRadius : "70";
  ...
  let doughnutDataTotal = 0;
  let resultDoughnutData = {
    type: "pie",
    radius: [`${doughnutInnerRadius}%`, `${doughnutOutSideRadius}%`],
  ...
  // 设置每一项和计算统计
  resultItem.forEach((doughnut, index) => {
    let doughnutColor = "";
    // 判断当前角标是否超出颜色数组长度
    if (index < colorList.length) {
      doughnutColor = colorList[index];
    }
    doughnutDataTotal += parseFloat(doughnut.value);
    resultDoughnutData.data.push({
      name: doughnut.name,
      value: doughnut.value,
      itemStyle: { color: doughnutColor }
    });
  });
  options.series.push(resultDoughnutData);
  // 设置环状图的圆角
  if (resultAttrs.roundedModel) {
    resultDoughnutData.itemStyle = {
      borderRadius: 15,
      borderColor: "#fff",
      borderWidth: 2
    };
