...
let defaultAttrs = computed(() => {
  return Object.assign({}, defaultEchartAttribute, areaAttribute);
});
...
async function initChart(resResultItem) {
  ...
  await setAreaData(options, resResultItem, resultAttrs);
  renderOptions.value = options;
  renderModel.value = resultAttrs.darkModel ? "dark" : "light";
  renderRef.value.initRenderChart();
}
// 设置面积图的各个值
async function setAreaData(options, resResultItem, resultAttrs) {
  let resultItem = [];
