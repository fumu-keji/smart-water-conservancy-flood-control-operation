...
let defaultAttrs = computed(() => {
  return Object.assign({}, defaultEchartAttribute, pieceAttribute);
});
...
async function initChart(resResultItem) {
  ...
  await setPieceData(options, resResultItem, resultAttrs);
  renderOptions.value = options;
  renderModel.value = resultAttrs.darkModel ? "dark" : "light";
  renderRef.value.initRenderChart();
}
// 设置面积图的各个值
async function setPieceData(options, resResultItem, resultAttrs) {
  let resultItem = [];
