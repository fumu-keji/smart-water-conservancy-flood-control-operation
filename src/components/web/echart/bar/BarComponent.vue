...
let defaultAttrs = computed(() => {
  return Object.assign({}, defaultEchartAttribute, barAttribute);
});
...
async function initChart(resResultItem) {
  ...
  await setBarData(options, resResultItem, resultAttrs);
  renderOptions.value = options;
  renderModel.value = resultAttrs.darkModel ? "dark" : "light";
  renderRef.value.initRenderChart();
}
// 设置面积图的各个值
async function setBarData(options, resResultItem, resultAttrs) {
  let resultItem = [];
